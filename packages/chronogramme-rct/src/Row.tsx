import {
	HALF,
	type Timeline as HTMLTimeline,
	UNIT,
} from "@chronogramme/chronogramme";
import { type SyntheticEvent, useCallback } from "react";
import { GroupRowProvider } from "./groupRowContext/GroupRowProvider";
import { RowItemsProvider } from "./rowItemsContext/RowItemsProvider";
import type {
	BaseGroup,
	BaseItem,
	ItemRenderer,
	RctToCoreGroup,
	RctToCoreItem,
	RowRenderer,
} from "./timeline";

export const Row = <
	TGroupIdKey extends string = "id",
	TGroupTitleKey extends string = "title",
	TGroupRightTitleKey extends string = "rightTitle",
	TItemIdKey extends string = "id",
	TItemGroupKey extends string = "group",
	TItemTitleKey extends string = "title",
	TItemDivTitleKey extends string = "title",
	TItemTimeStartKey extends string = "start_time",
	TItemTimeEndKey extends string = "end_time",
	TGroup extends BaseGroup<
		TGroupIdKey,
		TGroupTitleKey,
		TGroupRightTitleKey
	> = BaseGroup<TGroupIdKey, TGroupTitleKey, TGroupRightTitleKey>,
	TItem extends BaseItem<
		TItemIdKey,
		TItemGroupKey,
		TItemTitleKey,
		TItemDivTitleKey,
		TItemTimeStartKey,
		TItemTimeEndKey
	> = BaseItem<
		TItemIdKey,
		TItemGroupKey,
		TItemTitleKey,
		TItemDivTitleKey,
		TItemTimeStartKey,
		TItemTimeEndKey
	>,
	TRowData = undefined,
>({
	group,
	groupIndex,
	horizontalLineClassNamesForGroup,
	itemHeightRatio,
	itemRenderer,
	onClick,
	onContextMenu,
	onDoubleClick,
	rowData,
	rowRenderer: RowComponent,
	timeline,
}: {
	group: RctToCoreGroup<TGroup>;
	groupIndex: number;
	horizontalLineClassNamesForGroup: ((group: TGroup) => string[]) | undefined;
	itemHeightRatio: number;
	itemRenderer: ItemRenderer<
		TItemIdKey,
		TItemGroupKey,
		TItemTitleKey,
		TItemDivTitleKey,
		TItemTimeStartKey,
		TItemTimeEndKey,
		TItem
	>;
	onClick?:
		| ((groupId: number, time: number, e: SyntheticEvent) => void)
		| undefined;
	onContextMenu?:
		| ((groupId: number, time: number, e: SyntheticEvent) => void)
		| undefined;
	onDoubleClick:
		| ((groupId: number, time: number, e: SyntheticEvent) => void)
		| undefined;
	rowData: TRowData;
	rowRenderer: RowRenderer<
		TGroupIdKey,
		TGroupTitleKey,
		TGroupRightTitleKey,
		TGroup,
		TRowData
	>;
	timeline: InstanceType<
		typeof HTMLTimeline<
			number,
			RctToCoreGroup<TGroup>,
			number,
			RctToCoreItem<TItem>
		>
	>;
}): React.ReactNode => {
	const groupId = group.id;

	const lineSize = timeline.getGroupLineSize(groupIndex);
	const groupPosition = timeline.getGroupPosition(groupIndex);
	const groupSize = timeline.getGroupSize(groupIndex);
	const itemVOffset = HALF * (UNIT - itemHeightRatio) * lineSize;
	const itemVSize = itemHeightRatio * lineSize;

	const onClickHandler: (event: React.MouseEvent) => void = useCallback(
		(event) => {
			onClick?.(groupId, timeline.getHValueFromClient(event.clientX), event);
		},
		[groupId, onClick, timeline.getHValueFromClient],
	);

	const onContextMenuHandler: (event: React.MouseEvent) => void = useCallback(
		(event) => {
			onContextMenu?.(
				groupId,
				timeline.getHValueFromClient(event.clientX),
				event,
			);
		},
		[groupId, onContextMenu, timeline.getHValueFromClient],
	);

	const onDoubleClickHandler: (event: React.MouseEvent) => void = useCallback(
		(event) => {
			onDoubleClick?.(
				groupId,
				timeline.getHValueFromClient(event.clientX),
				event,
			);
		},
		[groupId, onDoubleClick, timeline.getHValueFromClient],
	);

	return (
		<GroupRowProvider<TGroupIdKey, TGroupTitleKey, TGroupRightTitleKey, TGroup>
			group={group.originalGroup}
			groupHeight={groupSize}
			groupPosition={groupPosition}
			horizontalLineClassNamesForGroup={horizontalLineClassNamesForGroup}
			isEvenRow={groupIndex % 2 === 0}
			key={`group-${groupId}`}
			onClick={onClickHandler}
			onContextMenu={onContextMenuHandler}
			onDoubleClick={onDoubleClickHandler}
		>
			<RowItemsProvider<
				TItemIdKey,
				TItemGroupKey,
				TItemTitleKey,
				TItemDivTitleKey,
				TItemTimeStartKey,
				TItemTimeEndKey,
				TItem
			>
				groupIndex={groupIndex}
				itemRenderer={itemRenderer}
				itemVOffset={itemVOffset}
				itemVSize={itemVSize}
				lineSize={lineSize}
			>
				<RowComponent
					getLayerRootProps={() => {
						return { style: {} };
					}}
					group={group.originalGroup}
					rowData={rowData}
				/>
			</RowItemsProvider>
		</GroupRowProvider>
	);
};
